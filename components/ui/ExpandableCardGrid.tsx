...
export interface ExpandableCardItem {
    ...
    badge?: string;
    featured?: boolean;
    /** Set false to use object-cover (screenshots). Default true = object-contain (certificates/docs) */
    isDocument?: boolean;
...
export function ExpandableCardGrid({ items }: { items: ExpandableCardItem[] }) {
    ...
    // Lock body scroll when expanded card is open
    useEffect(() => {
        ...
        <>
                ...
                {active && (
                    <div className="fixed inset-0 z-50 grid place-items-center p-4 sm:p-6">
                        ...
                        >
                            ...
                            {/* Image — aspect-ratio container prevents any layout shift */}
                            {active.src ? (
                                <div className="relative w-full aspect-[16/10] bg-bg-primary">
                                    <Image
                                        src={active.src}
                                        alt={active.title}
                                        fill
                                        ...
                                        className={`transition-opacity duration-300 ${
                                            // Certificates: contain so full document is visible
                                            // Screenshots: cover for full-bleed
                                            active.isDocument === false
                                ...
                                <div className={`aspect-[16/10] bg-gradient-to-br ${active.placeholderColor ?? DEFAULT_GRADIENTS[0]} flex items-center justify-center`}>
                            ...
                            <div className="p-5 sm:p-6">
                                <div className="flex items-start justify-between gap-4 mb-4">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <h3 className="text-base sm:text-lg font-bold text-text-primary leading-snug">{active.title}</h3>
            ...
            <ul className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                {items.map((item, idx) => (
                    ...
                    >
                        {/* Thumbnail — consistent aspect ratio prevents layout shifts */}
                        <div className="relative overflow-hidden">
                            {item.src ? (
                                ...
                                <div className={`aspect-[16/10] bg-gradient-to-br ${item.placeholderColor ?? DEFAULT_GRADIENTS[idx % DEFAULT_GRADIENTS.length]} flex items-center justify-center`}>
