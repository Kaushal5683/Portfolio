...
// Shared spring config — tighter for snappier feel
...
const dropdownTransition = { duration: 0.12, ease: [0.2, 0, 0.2, 1] };
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
    ...
    const scheduleClose = useCallback(() => {
        ...
        closeTimer.current = setTimeout(() => setDropdownOpen(null), 100);
    }, []);
                ...
                return (
                    ...
                    >
                        <Link
                            href={item.link}
                            onClick={onItemClick}
                            className="relative flex items-center gap-1 px-4 py-2 text-text-secondary hover:text-text-primary transition-colors duration-150"
                        >
                            {hovered === idx && (
                                <motion.div
                                    layoutId="nav-hover"
                            ...
                            {hasSub && (
                                <ChevronDown
                                    size={13}
                                    className={cn(
                                        "relative z-20 transition-transform duration-150",
                                    ...
                                    >
                                        ...
                                        {item.subItems!.map((sub, si) => (
                                            <Link
                                                ...
                                                className="flex items-center gap-3 px-4 py-2.5 text-sm text-text-secondary hover:text-text-primary hover:bg-white/5 transition-colors duration-100 group"
                                            >
                                                {sub.icon && (
                                                    <sub.icon size={15} className="text-brand-400 group-hover:text-brand-300 transition-colors duration-100 flex-shrink-0" />
                                                )}
                                                <span>{sub.name}</span>
                                            </Link>
                                        ))}
...
// ─── MobileNavToggle ─────────────────────────────────────────────────────────
export const MobileNavToggle = memo(({ isOpen, onClick }: { isOpen: boolean; onClick: () => void }) =>
    isOpen
